...
import { GripVertical, Plus, X, Save, Download, Building2, FileDown } from 'lucide-react';
...
export default function PatchPanelManager() {
  ...
  const loadData = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/patch/sites');
      ...
      if (data.length > 0) {
        ...
        const transformedSites = data.map(site => ({
          id: site.id,
          name: site.name,
          panels: (site.panels || []).map(panel => ({
            id: panel.id,
            name: panel.name,
            total_ports: panel.total_ports || 48,
            labels: (panel.labels || []).map(label => ({
              id: label.id || Date.now() + Math.random(),
        ...
        setActiveSiteId(transformedSites[0].id);
        if (transformedSites[0].panels && transformedSites[0].panels.length > 0) {
          setActivePanelId(transformedSites[0].panels[0].id);
        }
      } else {
        // Initialize with default site if empty
        const defaultSite = {
          id: 1,
          name: 'Main Building',
          panels: [
      ...
      // Initialize with default if backend fails
      const defaultSite = {
        id: 1,
        name: 'Main Building',
        panels: [
  ...
  const saveToBackend = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/api/patch/save', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ sites: transformedSites })
      });
  ...
  const exportPDF = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/patch/export-pdf/${activePanelId}`);
      const blob = await response.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `patch_panel_${activePanelId}.pdf`;
      a.click();
  ...
  const totalPorts = activePanel?.total_ports || 48;
  ...
  const deleteSite = async (id) => {
    ...
    // Confirm deletion
    if (!confirm(`Are you sure you want to delete "${sites.find(s => s.id === id)?.name}"?`)) {
      return;
    }
    ...
    if (activeSiteId === id) {
      const newActiveSite = updatedSites[0];
    ...
    try {
      const transformedSites = updatedSites.map(site => ({
        id: site.id,
        name: site.name,
        panels: site.panels.map(panel => ({
          id: panel.id,
      ...
      if (response.ok) {
        setSaveStatus('✓ Site deleted and saved!');
        setTimeout(() => setSaveStatus(''), 3000);
      } else {
        setSaveStatus('✗ Delete save failed');
        setTimeout(() => setSaveStatus(''), 3000);
      }
    } catch (error) {
      console.error('Save error after delete:', error);
      setSaveStatus('✗ Delete save failed');
      setTimeout(() => setSaveStatus(''), 3000);
    }
  };
      ...
      <div className="max-w-6xl mx-auto">
        <div className="bg-gray-800 rounded-lg shadow-2xl p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="bg-gray-700 rounded-lg p-4">
              ...
              <input
                type="number"
                value={totalPorts}
                onChange={(e) => updateActivePanel({ total_ports: Math.max(1, parseInt(e.target.value) || 0) })}
          ...
          <div className="flex gap-3 mb-6 items-center">
            ...
            {saveStatus && (
              <span className={`text-sm font-semibold ${saveStatus.includes('✓') ? 'text-green-400' : 'text-red-400'}`}>
                {saveStatus}
              </span>
            )}
          </div>
